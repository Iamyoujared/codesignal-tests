...
function solution(ride_time, ride_distance, cost_per_minute, cost_per_mile) {
  const total_costs_per_minute = [];
  for (let i = 0; i < cost_per_minute.length; i++) {
    total_costs_per_minute.push(cost_per_minute[i] * ride_time);
  }

  const total_costs_per_mile = [];
  for (let i = 0; i < cost_per_mile.length; i++) {
    total_costs_per_mile.push(
      cost_per_mile[i] * ride_distance + total_costs_per_minute[i]
    );
  }

  return total_costs_per_mile;
}
